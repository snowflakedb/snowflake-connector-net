#!/bin/bash -e
#
# Test Snowflake .NET Connector in Alpine Linux
#
# This script runs inside the Alpine Linux Docker container.
...
# Display musl libc info (this is the key differentiator for Alpine)
echo "[INFO] libc version:"
...
dotnet-coverage collect \
    "dotnet test --framework ${net_version} --no-build --logger \"junit;LogFilePath=alpine_${net_version}_${snowflake_cloud_env}_results.junit.xml\" --verbosity normal" \
    --output "alpine_${net_version}_${snowflake_cloud_env}_coverage.xml" \
