...
- v6.2.0
  ...
  - CommandTimeout and CancellationToken cancellation on non-PUT/GET queries now include a SnowflakeDbException as InnerException on the thrown OperationCanceledException. The inner exception carries the QueryId and error code (270007 REQUEST_TIMEOUT for timeout, QUERY_CANCELLED for external cancellation). Existing catch blocks for OperationCanceledException continue to work unchanged.
