...
namespace Snowflake.Data.Core.Authenticator.WorkflowIdentity
{
    internal class WorkflowIdentityAwsAttestationRetriever : WorkloadIdentityAttestationRetriever
    {
        ...
        private ImmutableCredentials AssumeRole(string targetRoleArn, string region, ImmutableCredentials credentials)
        {
            try
            {
                // Use provided credentials (could be attached identity or previously assumed role)
                var attachedCredentials = credentials;
                ...
                var queryParams = $"Action=AssumeRole&Version=2011-06-15&RoleArn={HttpUtility.UrlEncode(targetRoleArn)}&RoleSessionName={HttpUtility.UrlEncode(roleSessionName)}&DurationSeconds=3600";
                ...
                // Sign the request with the attached identity's credentials
                var awsConfiguration = new AwsConfiguration
                {
                    Region = region,
                    Service = "sts",
                ...
                using (var response = _restRequester.Get(new RestRequestWrapper(request)))
                {
                    ...
                    {
                        s_logger.Error($"AssumeRole request failed with status {response.StatusCode}: {responseXml}");
                        throw AttestationError($"Failed to assume role {targetRoleArn}: {response.StatusCode}");
                    }
                }
