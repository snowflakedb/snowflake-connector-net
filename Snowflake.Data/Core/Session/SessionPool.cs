...
namespace Snowflake.Data.Core.Session
{
    ...
    sealed class SessionPool : IDisposable
    {
        ...
        private bool ShouldPostponeMinPoolSize(SFSessionProperties properties)
        {
            ...
            if (MFACacheAuthenticator.IsMfaCacheAuthenticator(authenticator))
                return true;
            var isOAuthFlow = OAuthAuthorizationCodeAuthenticator.IsOAuthAuthorizationCodeAuthenticator(authenticator) ||
                              OAuthClientCredentialsAuthenticator.IsOAuthClientCredentialsAuthenticator(authenticator);
            if (isOAuthFlow)
                return bool.Parse(properties[SFSessionProperty.CLIENT_STORE_TEMPORARY_CREDENTIAL]) &&
                       !string.IsNullOrEmpty(properties[SFSessionProperty.USER]);
            return false;
        }
