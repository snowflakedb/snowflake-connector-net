...
namespace Snowflake.Data.Tests.UnitTests.Authenticator
{
    ...
    public class OAuthAuthorizationCodeFlowTest
    {
        ...
        public void TestSuccessfulAuthorizationCodeFlow()
        {
            ...
            Assert.AreEqual(AccessToken, SecureStringHelper.Decode(session._accessToken));
        ...
        public void TestRefreshToken()
        {
            ...
            // assert
            Assert.NotNull(session._accessToken);
            Assert.AreEqual(NewAccessToken, SecureStringHelper.Decode(session._accessToken));
            Assert.AreEqual(SessionId, session.sessionId);
            Assert.AreEqual(MasterToken, session.masterToken);
            Assert.AreEqual(SessionToken, session.sessionToken);
        ...
        private string GetAuthorizationCodeConnectionString()
        {
            ...
            return new StringBuilder()
                ...
                .Append($"external_authorization_url={s_externalAuthorizationUrl};external_token_request_url={s_externalTokenRequestUrl};")
                .ToString();
        }
