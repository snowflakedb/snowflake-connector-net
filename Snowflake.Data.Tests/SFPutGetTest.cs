...
namespace Snowflake.Data.Tests
{
    ...
    class SFPutGetTest : SFBaseTest
    {
        ...
        [IgnoreOnEnvIs("snowflake_cloud_env",
                       new string[] { "AWS" })]
        public void TestParallelFilesCommand()
        {
            string DATABASE_NAME = testConfig.database;
            string SCHEMA_NAME = testConfig.schema;
            const string TEST_TEMP_TABLE_NAME = "TEST_TEMP_TABLE_NAME";
            ...
            const string TEMP_FILE_PREFIX = "TEMP_FILES_FOR_PUT_GET";
            const int NUM_OF_FILES = 10;
            ...
            using (DbConnection conn = new SnowflakeDbConnection())
            {
                ...
                string filePrefix = Path.GetTempPath() + TEMP_FILE_PREFIX;
                string fileWildcardPaths = filePrefix + "*.csv";
                string[] filePaths = new string[NUM_OF_FILES];
                for (int i = 0; i < NUM_OF_FILES; i++)
                {
                    // Create a temp file with specified file extension
                    string filePath = filePrefix + Guid.NewGuid().ToString() + ".csv";
                    // Write row data to temp file
                    File.WriteAllText(filePath, ROW_DATA);
                ...
                string putQuery = $"PUT file://{fileWildcardPaths} @{DATABASE_NAME}.{SCHEMA_NAME}.%{TEST_TEMP_TABLE_NAME}";
                string getQuery = $"GET @{DATABASE_NAME}.{SCHEMA_NAME}.%{TEST_TEMP_TABLE_NAME} file://{tempDirectory}";
                ...
                if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                {
                    fileWildcard = fileWildcardPaths.Substring(fileWildcardPaths.LastIndexOf('\\') + 1);
                }
                else
                {
                    fileWildcard = fileWildcardPaths.Substring(fileWildcardPaths.LastIndexOf('/') + 1);
                ...
                if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                {
                    putQuery = $"PUT file://C:\\\\Users\\{Environment.UserName}\\AppData\\Local\\Temp\\{fileWildcard} @{DATABASE_NAME}.{SCHEMA_NAME}.%{TEST_TEMP_TABLE_NAME}";
                }
                ...
                using (DbCommand command = conn.CreateCommand())
                {
                    ...
                    // Check row count is correct
                    command.CommandText = $"SELECT COUNT(*) FROM {TEST_TEMP_TABLE_NAME}";
                    Assert.AreEqual(command.ExecuteScalar(), 40);
                    ...
                    while (reader.Read())
                    {
                        ...
                        // Check file contents
                        using (var streamReader = new StreamReader($@"{tempDirectory}/{reader.GetString(0)}"))
                        {
                            while (!streamReader.EndOfStream)
                            {
                                var line = streamReader.ReadLine();
