...
namespace Snowflake.Data.Tests.Util
{
    public sealed class WiremockRunner : IDisposable
    {
        ...
        public static WiremockRunner NewWiremock(string[] mappingFiles = null)
        {
            ...
            {
                Urls = new[] { $"http://{localhost}:{httpPort}", $"https://{localhost}:{httpsPort}" },
        ...
        private static IRequestBuilder BuildRequest(MappingRequest req)
        {
            ...
            {
                foreach (var pattern in req.BodyPatterns)
                {
                    var bodyMatcher = BuildBodyMatcher(pattern);
                    if (bodyMatcher != null)
                        builder = builder.WithBody(bodyMatcher);
                }
            }
        ...
        private static IStringMatcher[] BuildMatchers(MatcherSpec spec)
        {
            ...
            if (spec.EqualTo != null) return new IStringMatcher[] { new ExactMatcher(spec.EqualTo) };
            if (spec.Contains != null) return new IStringMatcher[] { new WildcardMatcher($"*{spec.Contains}*") };
            if (spec.Matches != null) return new IStringMatcher[] { new RegexMatcher(spec.Matches) };
