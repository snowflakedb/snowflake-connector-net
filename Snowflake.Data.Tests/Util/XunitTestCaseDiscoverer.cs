...
public sealed class XunitTestCaseDiscoverer : IXunitTestCaseDiscoverer
{
    ...
    {
        return _decorated.Discover(discoveryOptions, testMethod, factAttribute).Select(x => new XunitTestCaseDecorator(x));
    }
    ...
    public sealed class XunitTestCaseDecorator : LongLivedMarshalByRefObject, IXunitTestCase
    {
        ...
        public async Task<RunSummary> RunAsync(IMessageSink diagnosticMessageSink, IMessageBus messageBus, object[] constructorArguments, ExceptionAggregator aggregator,
            CancellationTokenSource cancellationTokenSource)
        {
            var messageBusDecorator = new  MessageBusDecorator(messageBus);
            var baseResult = await _xunitTestCaseImplementation.RunAsync(diagnosticMessageSink, messageBusDecorator, constructorArguments, aggregator, cancellationTokenSource).ConfigureAwait(false);
