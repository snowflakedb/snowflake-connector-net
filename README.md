...
| INSECUREMODE               |No        | Set to true to disable the certificate revocation list check. Default is false.<br /> :warning: Because the certificate revocation list check can only be set globally, each new connection erases the previous configuration for all http requests. Effectively the driver uses the value from the last connection made.|
