...
| MAXHTTPRETRIES             | No       | Maximum number of times to retry a failed HTTP requests (default: 7). You can can set `MAXHTTPRETRIES=0` to remove the retry limit, but doing so runs the risk of the .NET driver infinitely retrying failed HTTP calls. |
