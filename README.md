...
| MINPOOLSIZE                    | No       | Expected minimum number of connections in pool. When you get a connection from the pool, more connections might be initialised in background to increase the pool size to `minPoolSize`. The default value is 2. `maxPoolSize` value cannot be lower than `minPoolSize` value. The parameter is used only in a new version of connection pool.                                                                                                                                                                                                                                                                                                                                                                                               |
| CHANGEDSESSION                 | No       | Specifies what should happen to a session on which some of its parameters has changed (e. g. you used `ALTER SESSION SET SCHEMA` to change the databese schema). The default behaviour is `OriginalPool` which means the session stays in the original pool. Currently no other option is possible. Parameter used only in a new version of connection pool.                                                                                                                                                                                                                                                                                                                                                                                 |
| WAITINGFORIDLESESSIONTIMEOUT   | No       | Timeout for waiting for a session if there is no idle session and we cannot create a new one because of reaching `maxPoolSize`. The default value is 30 seconds. You can specify a time unit (ms - milliseconds, s - seconds, m - minutes) or use the default time unit which is seconds, e. g. `waitingForIdleSessionTimeout=5` or `waitingForIdleSessionTimeout=5s` indicates 5 seconds, `waitingForIdleSessionTimeout=1m` indicates 1 minute, `waitingForIdleSessionTimeout=50ms` indicates 50 milliseconds,  `waitingForIdleSessionTimeout=0` indicates that there will be no waiting for an idle session, using negative number  e. g. `waitingForIdleSessionTimeout=-1` indicates that there is no limitation for the time of waiting. |
| EXPIRATIONTIMEOUT              | No       | Timeout for using each connection. Connections which last more than specified timeout are considered to be expired and are being removed from the pool. The default is 1 hour. You can specify a time unit (ms - milliseconds, s - seconds, m - minutes) or use the default time unit which is seconds, e. g. `expirationTimeout=60m` indicates 60 minutes, `expirationTimeout=3600s` or `expirationTimeout=3600` indicates 3600 seconds, `expirationTimeout=360000ms` indicates 360000 milliseconds, `expirationTimeout=0` means that each connection after creation would be instantly expired. Expiration timeout cannot be set to infinity.                                                                                              |
